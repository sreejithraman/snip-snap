...
[[ -f "$checksum_file" ]] || fail "missing $checksum_file; run release.sh first"
release_policy_verify_checksum "$release_zip" "$checksum_file"
...
tap_checkout="$temp_root/homebrew-tap"
...
if gh release view "v$version" --repo "$release_repo" >/dev/null 2>&1; then
    ...
    release_policy_verify_checksum \
        "$existing_release_dir/Snip-Snap-$version.zip" \
        "$checksum_file" || fail "GitHub v$version has different release bytes"
...
checksum="$(/usr/bin/awk 'NR == 1 { print $1 }' "$checksum_file")"
...
if [[ -n "$(git -C "$release_checkout" status --short)" ]]; then
    git -C "$release_checkout" add appcast.xml
