...
Snip Snap sets a 25 MiB per attachment and 100 MiB per snip limit for iCloud Sync. These are Snip Snap limits, not Apple limits. Automated Mac and Simulator evidence sets this product policy. The release tests cover the inclusive boundaries, one-byte overflow, full incompatible-file reporting, and an interrupted 25 MiB upload and download with hash and cache checks on both iPhone and iPad Simulators. They also run the enable action on both device families. The short physical-iPhone transfer remains an unchecked release gate until a maintainer runs it.
