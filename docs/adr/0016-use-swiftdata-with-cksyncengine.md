...
The app may ask the engine to fetch or send now, while still accepting that scheduled work depends on system conditions. Only a main app may open the App Group SwiftData store. SwiftData can run an automatic migration even when no custom migration plan is supplied, so the Share extension will always write an atomic pending import instead. The main app will import that item exactly once when it next runs or returns to the foreground, then let normal sync handle it. The main app alone owns migrations and the sync engine.
...
The first public release still accepts the earlier text-only sync protocol used by pre-release team builds. This keeps test data created during iOS and CloudKit bring-up readable while those builds move to full-record sync. New stores always use full-record sync. Remove the legacy adapter after the first public release has given team devices a migration window; it is not a second supported sync choice.
