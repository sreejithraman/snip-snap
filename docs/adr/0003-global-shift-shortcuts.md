...
Treat two left Shift taps as Capture Selection. Treat two right Shift taps as Show or Hide Snip Snap, and focus the active section's composer when showing it. Keep clip keys active only while the clip list is the current command target.
...
The app must register and validate global key input, preserve user shortcut settings, and keep clip commands away from editors and text fields. A real cross-app check needs Accessibility access and a physical key press.
