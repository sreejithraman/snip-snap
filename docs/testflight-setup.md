...
- Platform: iOS.
- Name: Snip Snap, if Apple accepts the name.
- Primary language.
...
- Publish a public privacy policy URL. The policy should explain local-only
  mode, optional private-CloudKit sync, data access, retention, deletion, and
  how a user turns sync off or removes synced content. Apple requires the URL
  for iOS, and its review rules also require an easy-to-find privacy-policy link
  inside the app. [Manage app
...
- Make an export-compliance determination. CloudKit and operating-system
  encryption still require answering Apple's questions. If the final answer is
  that the app uses no non-exempt encryption, add
  `ITSAppUsesNonExemptEncryption = NO` to the app's `Info.plist` so each upload
  does not stop at **Missing Compliance**. Do not add that value until the team
  has made and recorded the determination. [Export compliance
...
2. **No export declaration exists in `SnipSnapiOS/Info.plist`.** Record the
   compliance decision, then add the correct key or provide documents through
   App Store Connect.
3. **No public privacy-policy link exists in the iOS app.** Publish the policy,
   add it to Settings, and enter the same URL in App Store Connect.
4. **The global Apple publishing profile has not been proved ready.** Keep its
