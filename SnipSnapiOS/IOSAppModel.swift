...
final class IOSAppModel {
    ...
    func createSnip(content: String, in listID: UUID) async -> Bool {
        await withSerializedMutation { await createSnipUnlocked(content: content, in: listID) }
    }

    @discardableResult
    func editSnip(_ snip: Snip, content: String) async -> Bool {
        await withSerializedMutation { await editSnipUnlocked(snip, content: content) }
    }
    ...
    private func createSnipUnlocked(content: String, in listID: UUID) async -> Bool {
    ...
    private func editSnipUnlocked(_ snip: Snip, content: String) async -> Bool {
        await performUndoable(
            .update(
                id: snip.id,
                content: content,
    ...
    private var currentSnapshot: SnipLibrarySnapshot {
        SnipLibrarySnapshot(snips: snips, lists: lists)
    }
