...
struct IOSUndoHistory {
    ...
    enum InverseEdit {
        ...
        case replace(before: Snip, expectedUpdatedAt: Date)
        ...
        func command(now: Date) -> SnipLibraryCommand {
            ...
            case .replace(let before, let expectedUpdatedAt):
                .restoreReplacing(
                    snips: [before],
                    id: before.id,
                    expectedUpdatedAt: expectedUpdatedAt
        ...
        var retainedAttachmentIDs: Set<UUID> {
            switch self {
            case .restore(let snips):
                Set(snips.flatMap(\.attachments).map(\.id))
            case .update(let before, _), .replace(let before, _):
                Set(before.attachments.map(\.id))
            default:
                []
            }
        }
    ...
    var retainedAttachmentIDs: Set<UUID> {
        operations.reduce(into: []) { result, operation in
            result.formUnion(operation.inverse.retainedAttachmentIDs)
        }
    }
